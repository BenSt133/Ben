...
def write_read_daq_fast(data_in,rate,board_num,ai_info,ai_range,ao_info,ao_range,low_chan=0,high_chan=0,low_chani=0,high_chani=0,outpad=100,num_channels_out=1,num_channels_in=1): #only 1 channel now
...
def addpre(datain,tau=10,nzeros=250,amppre=10.):
    t=np.linspace(0,100,250)
    return np.hstack((np.zeros(nzeros),amppre*np.ones(tau),np.zeros(len(t)-tau),datain))


def run_experiment(x,board_num,ai_info,ai_range,ao_info,
            ao_range,A=1,B=0,OD=56,Tmax=600,st=660,Nde=450,
            input_pad=100,checklength=3000,trigger=0.05,outpad=3000,channel=0):
    ...
    #time_i=np.linspace(0,Tmax,Ni)*dt
    ...
    for r in range(batch_size):
        fa=interp1d(time_i,x[r,:], kind="nearest", fill_value=(0.0,0.0),bounds_error=False)
        if r==0:
    ...
    data_in = addpre(signal_0,amppre=Amax)
    
    #plt.plot(addpre(signal_0,amppre=Amax))
    data_read=write_read_daq_fast(data_in,rate,board_num,ai_info,ai_range,ao_info,ao_range,low_chan=channel,high_chan=channel,low_chani=channel,high_chani=channel,outpad=outpad,num_channels_out=1,num_channels_in=1)
    ...
    Yp1=list()
    Yp2=list()
    out0=outs[0] #trigger source

    outy0=np.abs(out0[:checklength])
    ...
    try:
        for idx in range(len(outy0)):
            if timer[idx]>trigger and timer[idx+1]<=trigger:
                outy_start = idx
                break
    except:
        print('PNN trigger failed')
        return np.zeros(1),1,Y

    outy0=out0[outy_start+st:]
